...
<x-admin-layout>
    <div class="py-4 px-4 text-white flex justify-between items-center max-w-7xl bg-sky-800 border mx-auto mt-10 rounded-lg">
        ...
        @if(auth()->user()->is_admin ?? false)
            <button type="button" data-modal="user-create-modal" class="hover:cursor-pointer border-none bg-transparent p-0 m-0">
                        ...
                        <td class="px-4 py-4 w-fit">
                            <div class="bg-green-200 inline-block rounded-lg">
                                <button type="button"
                                    ...
                                    data-photopath="{{ $user->profile_photo_path }}"
                                >Ver</button>
                            </div>
                            @if(auth()->user()->id == $user->id || auth()->user()->is_admin ?? false)
                                <div class="bg-blue-200 inline-block rounded-lg">
                                    <button type="button"
                                        ...
                                        data-modal="user-edit-modal"
                                        data-id="{{ $user->id }}"
                                        data-name="{{ $user->name }}"
                                        data-email="{{ $user->email }}"
                                        data-cpf="{{ $user->cpf }}"
                                        data-birth_date="{{ $user->birth_date ? \Carbon\Carbon::parse($user->birth_date)->format('Y-m-d') : '' }}"
                                        data-phone="{{ $user->phone }}"
