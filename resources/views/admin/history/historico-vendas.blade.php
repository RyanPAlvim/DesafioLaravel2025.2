    ...
    <div class="min-h-screen bg-sky-800 py-10 px-2 sm:px-8">
        <div class="max-w-5xl mx-auto bg-white/90 rounded-xl shadow-lg p-6">
            <div class="flex flex-col sm:flex-row sm:justify-between items-center mb-6 gap-4">
                ...
                <a href="{{ route('admin.history.vendas.pdf') }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-700 text-white font-semibold rounded-lg shadow transition">
                    ...
                    <tbody class="divide-y divide-blue-100">
                        @foreach($orderItems as $item)
                            <tr class="hover:bg-blue-50 transition">
                                ...
                                <td class="py-2 px-4 text-sky-800">{{ $item->order && $item->order->created_at ? $item->order->created_at->format('d/m/Y H:i') : '-' }}</td>
                                ...
                                @endif
                                <td class="py-2 px-4">{{ $item->order && $item->order->user ? $item->order->user->name : '-' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="text-center py-12">
                ...
                <p class="text-blue-700 text-lg">Nenhuma venda encontrada.</p>
            </div>
            @endif
        </div>
    </div>
