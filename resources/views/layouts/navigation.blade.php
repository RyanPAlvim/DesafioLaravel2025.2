<nav x-data="{ open: false }" class=" pt-5 bg-blue-500 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        @if(!(auth()->user()->is_admin ?? false))
                            <div class="px-4 py-2 text-green-700 font-bold text-center">
                                Saldo: R$ {{ number_format(auth()->user()->saldo, 2, ',', '.') }}
                            </div>
                            <hr class="my-2">
                        @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('home')" :active="request()->routeIs('dashboard')" class="text-white bg-transparent hover:bg-blue-700">
                {{ __('Página Inicial') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('admin.products.index')" :active="request()->routeIs('admin.products.index')" class="text-white bg-transparent hover:bg-blue-700">
                {{ __('Produtos') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.index')" class="text-white bg-transparent hover:bg-blue-700">
                @if(auth()->user()->is_admin ?? false)
                    {{ __('Usuários') }}
                @else
                    {{ __('Minha Conta') }}
        ...
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4">
                @if(!(auth()->user()->is_admin ?? false))
                    <div class="py-2 text-green-400 font-bold text-center">
                        Saldo: R$ {{ number_format(auth()->user()->saldo, 2, ',', '.') }}
                    </div>
                @endif
